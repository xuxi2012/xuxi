local aaa = loadstring(game:HttpGet "https://raw.githubusercontent.com/xuxi2012/xuxi/refs/heads/main/%E8%84%9A%E6%9C%AC.lua")()
...
spawn(function()
    while task.wait() do
        game:GetService("Workspace").Stores.WoodRUs.Parts.PREMIUMSELECTION.SurfaceGui.TextLabel.Text = "徐付费脚本"
        ...
        game:GetService("Workspace").Stores.WoodRUs.Parts.PREMIUMSELECTION.SurfaceGui.TextLabel.TextColor3 =
            Color3.fromRGB(0, 255, 255)
        wait(1)
        game:GetService("Workspace").Stores.WoodRUs.Parts.PREMIUMSELECTION.SurfaceGui.TextLabel.Text = "徐脚本"
        game:GetService("Workspace").Stores.WoodRUs.Parts.PREMIUMSELECTION.SurfaceGui.TextLabel.TextColor3 =
...
game:GetService("Workspace").Stores.WoodRUs.Parts.OPEN24HOURS.SurfaceGui.TextLabel.Text = "作者: 小徐";
game:GetService("Workspace").Stores.WoodRUs.Parts.OPEN24HOURS.SurfaceGui.TextLabel.TextColor3 =
    Color3.fromRGB(0, 255, 255)
game:GetService("Workspace").Stores.WoodRUs.Parts.SELLWOOD.SurfaceGui.TextLabel.Text = "作者: 小徐";
game:GetService("Workspace").Stores.WoodRUs.Parts.SELLWOOD.SurfaceGui.TextLabel.TextColor3 = Color3.fromRGB(0, 255, 255)
game:GetService("Workspace").Stores.WoodRUs.Parts.WOODDROPOFF.SurfaceGui.TextLabel.Text = "感谢使用徐";
...
local function donate(plr, amount)
    ...
    notify('徐', '转钱' .. tostring(amount) .. ' 给 ' .. tostring(plr), 4);
    scr2.exitNotice();
end
...
local window = library:new("徐--伐木大亨2", '')
...
Section:Button("获取当前死亡后加载时间", function()
    bailib:Notification("徐", " 时间为" .. bai.loaddupeaxewaittime .. "秒", "好")
end)
...
Section5:Toggle("大力", 'Toggleflag', false, function(state)
    if state then
...
Sectionbringtree:Dropdown("选择树", 'Dropdown',
    {"普通树", '幻影木', "沼泽黄金", "樱花", "蓝木", "冰木", "火山木", "橡木", "巧克力木",
     "徐桦木", "黄金木", "雪地松", "僵尸木", "大巧克力树", "椰子树", '南瓜木', '幽灵木'},
    function(b)
        if b == '普通树' then
...
local drop = Sectionzhenglimt:Dropdown("选择木头类型", 'Dropdown',
    {"普通树", "沼泽黄金", "樱花", "蓝木", "冰木", "火山木", "橡木", "巧克力木", "徐桦木",
...
Sectionhuanjin:Toggle("终日徐天", 'Toggleflag', false, function(state)
    if state then
        bai.awaysday = true
        while task.wait() do
...
Sectionhuanjin:Button("船离开时间", function()
    ...
    if game.Workspace.Ferry.TimeToDeparture.Value == 0 then
        ...
        notify("徐", "距离船离开时间有" .. game.Workspace.Ferry.TimeToDeparture.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("桥上升时间", function()
    ...
    if game.Workspace.Bridge.DownTime.Value < 0 then
        ...
        notify("徐", "距离桥上升的时间有" .. game.Workspace.Bridge.DownTime.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("查看游戏时间", function()
    notify("徐", "游戏时间为" .. tostring(game.Lighting.TimeOfDay):sub(1, -2):sub(1, -2):sub(1, -2), 4)
end)
...
Sectionautobuy6:Button("买鲨鱼", function()
    ...
    bai.boxOpenConnection = workspace.PlayerModels.ChildAdded:Connect(function(Child)
       ...
       player:Kick("你没有加入白名单，请加作者qq:3998396522")
    end
end
